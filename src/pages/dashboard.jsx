...
const Dashboard = () => {
  ...
  const grabTokens = async () => {
    ...
    const fetchedTokens = await fetch(
      `https://deep-index.moralis.io/api/v2/${address}/erc20?chain=eth&format=decimal`,
      { headers: { "X-API-Key": import.meta.env.VITE_MORALIS_KEY } }
    ).then((res) => res.json());
    const fetchedEther = await fetch(
        ...
        <Stack direction={{ base: "column" }} w="full">
          ...
          {erc20Tokens.map((token, pid) => (
            <React.Fragment key={pid}>
              ...
              {erc20Tokens[pid] !== erc20Tokens[erc20Tokens.length - 1] ? (
